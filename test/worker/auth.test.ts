...
describe('legacy data claim', () => {
  it('the first registration after a fresh deploy inherits pre-accounts data; later registrations get a clean slate', async () => {
    ...
    await db.batch([
      ...
      db.prepare(
        `INSERT INTO settings (user_id, full_time_weekly_minutes, workload_percent_x10,
         ...
         VALUES (0, 2520, 900, 504, 504, 504, 504, 504, 0, 0)`,
      ),
    ]);
    ...
    await expect(firstSettings.json()).resolves.toMatchObject({ workloadPercentX10: 900 });
    ...
    await expect(secondSettings.json()).resolves.toMatchObject({ workloadPercentX10: 1000 }); // default, not claimed
  });
});
